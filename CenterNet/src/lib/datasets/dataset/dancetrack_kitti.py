...
class DanceTrackKITTI(data.Dataset):
    num_classes = 3
#     default_resolution = [384, 1280]
    default_resolution = [544, 960]
    mean = np.array([0.485, 0.456, 0.406], np.float32).reshape(1, 1, 3)
    std = np.array([0.229, 0.224, 0.225], np.float32).reshape(1, 1, 3)
    calib = np.array([[707.0493, 0, 960/2, 0],
                      [0, 707.0493, 544/2, 0],
                      [0, 0,        1.,    0]], dtype=np.float32)
    def __init__(self, opt, split):
        super(DanceTrackKITTI, self).__init__()
        self.data_dir = os.path.join(opt.data_dir, 'dancetrack_kitti')
        self.img_dir = os.path.join(self.data_dir)
        if split == 'train':
            self.annot_path = os.path.join(self.data_dir, 'annotations', 'train.json')
        else:
            assert 0, 'only train is supported!'
        self.max_objs = 50
        self.class_name = [
        ...
        print('==> initializing dancetrack_kitti {} data.'.format(split))
