...
class DddDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        ########### avoid big padding when joint-training of dancetrack and kitti ##########
        black_flag = False
        black_margin = 0
        
        black_height, black_witdh = img.shape[:2]
        if index < 7481 and self.opt.remove_black:
            black_margin = int((black_witdh  - black_height / 544 * 960) / 2)
            img = img[:, black_margin:-black_margin, :]
            black_flag = True
        ####################################################################################
        ...
        dep_mask = np.zeros((self.max_objs), dtype=np.uint8)
        dim_mask = np.zeros((self.max_objs), dtype=np.uint8)
        ...
        for k in range(num_objs):
            ...
            ########### avoid big padding when joint-training of dancetrack and kitti ##########
            if black_flag:
                if bbox[2] < black_margin or bbox[0] > width + black_margin:
                    continue
                else:
                    bbox[[0, 2]] = bbox[[0, 2]] - black_margin 
            ####################################################################################
            ...
            if h > 0 and w > 0:
                ...
                wh[k] = 1. * w, 1. * h
                if 'alpha' in ann and 'depth' in ann and 'dim' in ann:
                    ...
                    if 1:
                        ...
                        dep_mask[k] = 1 if not aug else 0
                        dim_mask[k] = 1 if not aug else 0
                        rot_mask[k] = 1
                else:
                    ind[k] = ct_int[1] * self.opt.output_w + ct_int[0]
                    reg[k] = ct - ct_int
        ...
        ret = {'input': inp, 'hm': hm, 'dep': dep, 'dim': dim, 'ind': ind,
               ...
               'rot_mask': rot_mask, 'dep_mask': dep_mask, 'dim_mask': dim_mask}
        if self.opt.reg_bbox:
            ret.update({'wh': wh})
